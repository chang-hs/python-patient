...
import psycopg2
import subprocess
from myforms import PtRegisterForm, OpRegisterForm, IdForm, MajordivForm
from myforms import PathodivForm, DiseaseNameForm
from myforms import DiseaseNameWithNewForm, LocationForm, PatientIdForm
from myforms import PatientNameForm, SearchKeyForm
from myforms import LocationWithNewForm, OpSearchForm, OpIdForm, OpDisplayForm
from myforms import LoginForm, OpEditForm
...
def op_register():
    ...
    try:
        ...
        return render_template(
            "message.html",
            message="Unable to obtain patient name from the id: " + patient_id,
        )
    form = OpRegisterForm()
    ...
    if form.validate_on_submit():
        ...
        sql = (
            ...
            "start_time, end_time, indication, op_note, surgeons, assistants) "
        ...
        # Retrieve op_id from the data just inserted,
        # using op_date and start_time as queues
        ...
        session["op_id"] = op_id
        return render_template(
            "message_with_url.html",
            message="Op_id: " + str(op_id) + "  Data Successfully Inserted",
...
def location_register():
    ...
    try:
        cur.execute(
            sql,
            (
                session["major_div_id"],
                session["patho_div_id"],
                session["disease_name_id"],
            ),
        )
    except psycopg2.Error:
        return render_template("message.html", message="Database execution error")
...
def database_insertion():
    ...
    conn.close()
    return render_template(
        "display_after_op_diag_insertion.html", op_id=op_id, disease_id=disease_id
    )
...
def diag_adm_get_disease_name():
    ...
    try:
        ...
        return render_template(
            "message.html", message="Can't retrieve all disease names"
        )
    all_list[0:0] = [("0", "")]
    ...
    if disease_name_with_new_form.validate_on_submit():
        ...
        if new_disease_name != "":
            ...
            try:
                ...
                render_template(
                    "message.html", message="Failed to retrieve the new disease name"
                )
            ...
            return render_template(
                "message.html", message="Can't decide a disease_name_id"
            )
    # Initial display
    return render_template(
        "disease_name_with_new.html", form=disease_name_with_new_form
    )
...
def diag_adm_get_location():
    ...
    try:
        ...
        return render_template(
            "message.html", message="Can't retrieve all location names"
        )
    all_list[0:0] = [("0", "")]
    ...
    if location_with_new_form.validate_on_submit():
        ...
        if new_location_name != "":
            ...
            try:
                ...
                render_template(
                    "message.html", message="Failed to retrieve the new location"
                )
            ...
            if location_from_all_id in pertinent_list:
                return render_template(
                    "message.html", message="Location already exists!"
                )
...
def diag_adm_set_diag():
    ...
    sql = "INSERT INTO diagnosis (major_div_id, patho_div_id, location_id, \
            ...
            (%s, %s, %s, %s)"
    try:
        cur.execute(sql, (major_div_id, patho_div_id, location_id, disease_name_id))
        conn.commit()
    except psycopg2.Error:
        return render_template("message.html", message="Can't insert into diagnosis")
    return render_template(
        "diag_adm_success.html",
        major_div_id=major_div_id,
        patho_div_id=patho_div_id,
...
def search_patients_from_name_key():
    ...
    sql_result = Patient.query.filter(
        Patient.kanji_name.like("%" + name_key + "%")
    ).all()
...
def edit_op(op_id):
    ...
    if form.validate_on_submit():
        ...
        def get_current_surgeon_id_list(model):
            temp_list = db_session.query(model).filter(model.op_id == op_id).all()
            return [x.surgeon_id for x in temp_list]

        current_surgeon_id_list = get_current_surgeon_id_list(OpSurgeon)
        current_assistant_id_list = get_current_surgeon_id_list(OpAssistant)

        surg_ids_to_be_deleted = set(current_surgeon_id_list) - set(surgeon_ids)
        surg_ids_to_be_added = set(surgeon_ids) - set(current_surgeon_id_list)

        assist_ids_to_be_deleted = set(current_assistant_id_list) - set(assistant_ids)
        assist_ids_to_be_added = set(assistant_ids) - set(current_assistant_id_list)

        for model, delete_ids, add_ids in [
            (OpSurgeon, surg_ids_to_be_deleted, surg_ids_to_be_added),
        ...
        return render_template("message.html", message="Op data successfully edited")
...
def render_pdf_opnote_noid(op_id):
    ...
    with open("/home/chang/tmp/opnote.tex", "wt") as texfile:
        texfile.write(pdf_text)
    res = subprocess.call(
        "/home/chang/texlive/2022/bin/x86_64-linux/platex -output-directory /home/chang/tmp /home/chang/tmp/opnote.tex",
        shell=True,
    )
    res = subprocess.call(
        "/home/chang/texlive/2022/bin/x86_64-linux/dvipdfmx -o /home/chang/tmp/opnote.pdf /home/chang/tmp/opnote",
        shell=True,
    )
    cur.close()
    conn.close()
...
def op_search_from_key():
    ...
    sql = (
        "SELECT o.op_id, o.op_date, p.patient_id, p.kanji_name, o.procedure "
    ...
    result_list = [
        (
            ...
            url_for("display_op", op_id=item[0]),
        )
        for item in results
    ]
    return render_template(
        "display_opsearch_result.html", sql=sql, result_list=result_list
    )
...
def display_op_id_list():
    ...
    sql = (
        "SELECT p.patient_id, p.kanji_name, o.op_id, o.op_date, "
        ...
        op_rows.append(
            {
                ...
                "preop_dx": row[4],
                "procedure": row[5],
                "url": url_for("render_pdf_opnote", op_id=row[2]),
                "edited": edited_exists,
                "dicom": dicom_exists,
